...
def corr_discr(mdf: float, smdf: float, m: float, m40: float = M40):
    ...
    k0 = 1 / (delta_mass * mdf - delta_mass + 1) if (delta_mass * mdf - delta_mass + 1) != 0 else 0
    k1 = error_div((1, 0), (delta_mass * mdf - delta_mass + 1, smdf * delta_mass))
    return k0, k1
...
def calc_age(a0: float, e0: float, J: float, sJ: float, L: float, sL: float, uf: int = 1,
             useDecayConst: bool = False, recalibrationPrimary: bool = False,
             usePrimaryAge: bool = False, usePrimaryRatio: bool = False, **kwargs):
    """
