...
Sample Functions
"""
from pyarar import calcFuncs
from pyarar import plotFuncs
...
def initializeData(smp: sample.Sample):
    ...
    def _initialize(a0):
        return type(a0)()
    '''
    _dataList = [smp.MSequenceList, smp.BSequenceList, smp.MStepsList, smp.BStepsList,
                 smp.Ar36MList, smp.Ar37MList, smp.Ar38MList, smp.Ar39MList, smp.Ar40MList,
                 smp.Ar36MErrorList, smp.Ar37MErrorList, smp.Ar38MErrorList, smp.Ar39MErrorList,
                 smp.Ar40MErrorList,
                 smp.Ar36BList, smp.Ar37BList, smp.Ar38BList, smp.Ar39BList, smp.Ar40BList,
                 smp.Ar36BErrorList, smp.Ar37BErrorList, smp.Ar38BErrorList, smp.Ar39BErrorList,
                 smp.Ar40BErrorList,
                 smp.Ar36DegasCa, smp.Ar36DegasK, smp.Ar36DegasCl, smp.Ar36DegasAir,
                 smp.Ar37DegasCa, smp.Ar37DegasK, smp.Ar37DegasCl, smp.Ar37DegasAir,
                 smp.Ar38DegasCa, smp.Ar38DegasK, smp.Ar38DegasCl, smp.Ar38DegasAir,
                 smp.Ar39DegasCa, smp.Ar39DegasK, smp.Ar39DegasCl, smp.Ar39DegasAir,
                 smp.Ar40DegasCa, smp.Ar40DegasK, smp.Ar40DegasCl, smp.Ar40DegasAir,
                 smp.Ar40DegasR,
                 smp.Ar36DegasCaError, smp.Ar36DegasKError, smp.Ar36DegasClError, smp.Ar36DegasAirError,
                 smp.Ar37DegasCaError, smp.Ar37DegasKError, smp.Ar37DegasClError, smp.Ar37DegasAirError,
                 smp.Ar38DegasCaError, smp.Ar38DegasKError, smp.Ar38DegasClError, smp.Ar38DegasAirError,
                 smp.Ar39DegasCaError, smp.Ar39DegasKError, smp.Ar39DegasClError, smp.Ar39DegasAirError,
                 smp.Ar40DegasCaError, smp.Ar40DegasKError, smp.Ar40DegasClError, smp.Ar40DegasAirError,
                 smp.Ar40DegasRError]
    '''
    for key, value in smp.__dict__.items():
...
def readDataFromRawFile(smp: sample.Sample, path=None):
    ...
    res = calcFuncs.open_original_xls(smp.RawFilePath)
    if res:
        pass
...
def readDataFromFilteredFile(smp: sample.Sample, path=None):
    ...
    res = calcFuncs.open_filtered_xls(smp.FilteredFilePath)
    if res:
...
def readDataFromAgeFile(smp: sample.Sample, path=None):
    ...
    res = calcFuncs.open_age_xls(smp.AgeFilePath)
    if res:
        _readFilteredFile(smp, res)
        ...
        """read select state of each experiment steps"""
        ...
        smp.Ar40MassError = float(logs01_params[1][12]) / 100 * float(logs01_params[1][11])
        ...
        smp.UseMinCalculation = 1 if logs01_params[1][5] == 'MIN' else 0
        smp.CorrBlank = 1

def _readFilteredFile(smp, res):
    for key, value in res[0].items():
        smp.MSequenceList.append(value[0])
        smp.MStepsList.append(value[1])
        smp.Ar36MList.append(value[2])
        smp.Ar37MList.append(value[5])
        smp.Ar38MList.append(value[8])
        smp.Ar39MList.append(value[11])
        smp.Ar40MList.append(value[14])
        smp.Ar36MErrorList.append(value[3])
        smp.Ar37MErrorList.append(value[6])
        smp.Ar38MErrorList.append(value[9])
        smp.Ar39MErrorList.append(value[12])
        smp.Ar40MErrorList.append(value[15])
        smp.MDateTimeList.append([int(_i) for _i in value[17:22]])
    for key, value in res[1].items():
        smp.BStepsList.append(value[1])
        smp.BSequenceList.append(value[2])
        smp.Ar36BList.append(value[3])
        smp.Ar37BList.append(value[5])
        smp.Ar38BList.append(value[7])
        smp.Ar39BList.append(value[9])
        smp.Ar40BList.append(value[11])
        smp.Ar36BErrorList.append(value[4])
        smp.Ar37BErrorList.append(value[6])
        smp.Ar38BErrorList.append(value[8])
        smp.Ar39BErrorList.append(value[10])
        smp.Ar40BErrorList.append(value[12])
...
def corrBlank(smp: sample.Sample):
    ...
    if smp.CorrBlank:
        smp.Ar36List, smp.Ar36ErrorList \
            = calcFuncs.corr_blank(smp.Ar36MList, smp.Ar36MErrorList, smp.Ar36BList, smp.Ar36BErrorList)
        smp.Ar37List, smp.Ar37ErrorList \
            = calcFuncs.corr_blank(smp.Ar37MList, smp.Ar37MErrorList, smp.Ar37BList, smp.Ar37BErrorList)
        smp.Ar38List, smp.Ar38ErrorList \
            = calcFuncs.corr_blank(smp.Ar38MList, smp.Ar38MErrorList, smp.Ar38BList, smp.Ar38BErrorList)
        smp.Ar39List, smp.Ar39ErrorList \
            = calcFuncs.corr_blank(smp.Ar39MList, smp.Ar39MErrorList, smp.Ar39BList, smp.Ar39BErrorList)
        smp.Ar40List, smp.Ar40ErrorList \
            = calcFuncs.corr_blank(smp.Ar40MList, smp.Ar40MErrorList, smp.Ar40BList, smp.Ar40BErrorList)
...
def corrDiscr(smp: sample.Sample):
    ...
    """
    def _func(a0, e0, f):
        k1 = [calcFuncs.error_mul((a0[i], e0[i]), (f[0], f[1])) for i in range(len(a0))]
...
def corrDecay(smp: sample.UnkSample or sample.MonitorSample):
    ...
    """
    def _func(a0, e0, f):
        k1 = calcFuncs.error_mul((a0, e0), (f[0], f[1]))
        k0 = a0 * f[0]
        return k0, k1
    # Decay Correction
    ...
    if smp.Corr37ArDecay:
        for row in range(len(smp.Ar37List)):
            c = calcFuncs.corr_decay(smp.MDateTimeList[row], t2, t3, smp.Ar37Const, smp.Ar37ConstError)  # 37Ar
    ...
    if smp.Corr39ArDecay:
        for row in range(len(smp.Ar39List)):
            c = calcFuncs.corr_decay(smp.MDateTimeList[row], t2, t3, smp.Ar39Const, smp.Ar39ConstError)  # 39Ar
...
def degasPattern(smp: sample.UnkSample or sample.MonitorSample):
    ...
    def _mul(a0, e0, f, sf, state: bool):
        k1 = [calcFuncs.error_mul((a0[i], e0[i]), (f, sf)) if state else 0 for i in range(len(a0))]
        k0 = [a0[i] * f if state else 0 for i in range(len(a0))]
        return k0, k1

    def _sub(a0, e0, a1, e1, state: bool):
        k1 = [calcFuncs.error_add(e0[i], e1[i]) if state else 0 for i in range(len(a0))]
    ...
    def _second(datetime: list, endtime: list):
        ...
        return [(calcFuncs.get_datetime(t_year, t_month, t_day, t_hour, t_min) - i) / (3600 * 24 * 365.242) for i in
                endtime]
    # Corr Ca
    ...
    try:
        ...
        sv36acl = [calcFuncs.error_add(smp.Ar36ErrorList[i], smp.Ar36DegasCaError[i]) for i in
        ...
        sv38acl = [calcFuncs.error_add(smp.Ar38ErrorList[i], smp.Ar38DegasKError[i], smp.Ar38DegasCaError[i])
        ...
        s3 = [calcFuncs.error_div((1, 0), (v3[i], s3[i])) for i in range(len(s3))]
        ...
        '''
        # 38ArCl
        smp.Ar38DegasClError = [calcFuncs.error_mul((smp.Ar36DegasCl[i], smp.Ar36DegasClError[i]), (v3[i], s3[i]))
                                for i in range(len(smp.Ar36DegasCl))]
    ...
    try:
        ...
        smp.Ar36DegasAirError = [
            calcFuncs.error_add(smp.Ar36ErrorList[i], smp.Ar36DegasCaError[i], smp.Ar36DegasClError[i],
        ...
        smp.Ar38DegasAirError = [
            calcFuncs.error_mul((smp.Ar36DegasAir[i], smp.Ar36DegasAirError[i]),
    ...
    smp.Ar40DegasRError = [
        calcFuncs.error_add(smp.Ar40ErrorList[i], smp.Ar40DegasCaError[i], smp.Ar40DegasClError[i],
...
def calcApparentAge(smp: sample.UnkSample):
    ...
    smp.FValuesError = [
        calcFuncs.error_div((smp.Ar40DegasR[i], smp.Ar40DegasRError[i]), (smp.Ar39DegasK[i], smp.Ar39DegasKError[i]))
...
def calcKCaClRatios(smp: sample.UnkSample):
    def _func(a0: list, e0: list, a1: list, e1: list, f: float, sf: float):
        ...
        k1 = [calcFuncs.error_mul((f, sf), (a0[i] / a1[i], calcFuncs.error_div((a0[i], e0[i]), (a1[i], e1[i]))))
    ...
    if isKCa:
        ...
        smp.KCaRatios, smp.KCaRatiosError = \
            _func(smp.Ar37DegasCa, smp.Ar37DegasCaError, smp.Ar39DegasK, smp.Ar39DegasKError,
                  1 / smp.KvsCaFactor, calcFuncs.error_div((1, 0), (smp.KvsCaFactor, smp.KvsCaFactorError)))
    isKCl = True
    if isKCl:
        """K/Cl"""
        ...
        smp.KClRatios, smp.KClRatiosError = \
            _func(smp.Ar38DegasCl, smp.Ar38DegasClError, smp.Ar39DegasK, smp.Ar39DegasKError,
                  1 / smp.KvsClFactor, calcFuncs.error_div((1, 0), (smp.KvsClFactor, smp.KvsClFactorError)))
    isCaCl = True
    if isCaCl:
        """Ca/Cl"""
        ...
        smp.CaClRatios, smp.CaClRatiosError = \
            _func(smp.Ar38DegasCl, smp.Ar38DegasClError, smp.Ar37DegasCa, smp.Ar37DegasCaError,
                  1 / smp.KvsClFactor, calcFuncs.error_div((1, 0), (smp.CavsClFactor, smp.CavsClFactorError)))
...
def calcRatios(smp: sample.UnkSample):
    ...
    def _getIsochron(x, sx, y, sy, z, sz):
        ...
        k1 = [calcFuncs.error_div((x[i], sx[i]), (z[i], sz[i])) if z[i] != 0 else 0 for i in range(_n)]
        # y / z
        k2 = [y[i] / z[i] if z[i] != 0 else 0 for i in range(_n)]
        k3 = [calcFuncs.error_div((y[i], sy[i]), (z[i], sz[i])) if z[i] != 0 else 0 for i in range(_n)]
        pho = [calcFuncs.error_cor(sx[i] / x[i], sy[i] / y[i], sz[i] / z[i]) if x[i] * y[i] * z[i] != 0 else 0
               for i in range(_n)]
    ...
    c = [smp.Ar40List[i] - smp.Ar40DegasK[i] - smp.Ar40DegasCa[i] - smp.Ar40DegasCl[i]
         for i in range(len(smp.Ar40List))]
    sc = [calcFuncs.error_add(smp.Ar40ErrorList[i], smp.Ar40DegasKError[i], smp.Ar40DegasCaError[i],
...
def calcAge(F, sF, smp: sample.UnkSample):
    k0 = calcFuncs.calc_age(F, sF, smp.JValue, smp.JValueError, smp.K40Const, smp.K40ConstError,
                            uf=1, useMin2000=smp.UseMinCalculation,
                            ...
                            standard_Ar_conc=smp.Ar40Concentration,
                            standard_Ar_conc_error=smp.Ar40ConcentrationError,
                            standard_K_conc=smp.KConcentration, standard_K_conc_error=smp.KConcentrationError,
                            standard_40K_K=smp.StandardAr40vsK, standard_40K_K_error=smp.StandardAr40vsKError)
    return k0
...
def calcIsochronAge(smp: sample.UnkSample):
    def _getIsochronAge(iso, isNormal=False, isInverse=False, isKIsochron=False):
        ...
        try:
            k0 = calcFuncs.isochron_regress(x, sx, y, sy, pho,
            ...
            print("Error in ProFunctions.isochron_regress: {}".format(e))
            return ['Null'] * 16
...
def calcPlateauAge(smp: sample.UnkSample):
    def _plateau(points: list):
        F, sF = [[i[j] for j in points] for i in [smp.FValues, smp.FValuesError]]
        k4, k5, k6, k7 = calcFuncs.err_wtd_mean(F, sF)  # weighted F, error, dp, MSWD
    ...
    smp.PlateauAges = []
    x, y1, y2 = calcFuncs.get_spectra(smp.ApparentAge, smp.ApparentAgeInternalError, smp.Ar39DegasK)
...
def calcTFAge(smp: sample.UnkSample):
    ...
    F, sF = a0 / a1, calcFuncs.error_div((a0, e0), (a1, e1))
...
def calc3DIsochronAge(smp: sample.UnkSample):
    ...
    try:
        k0 = calcFuncs.intercept_linest(_pick(z), _pick(x), _pick(y))  # z = k[0] + k[5][0] * x + k[5][1] * y
        if not k0:
            raise ValueError("raise error")
    except Exception as e:
...
def calcJValue(smp: sample.MonitorSample):
    smp.MonitorJValue, smp.MonitorJValueError = \
        calcFuncs.j_value(smp.MonitorAge, smp.MonitorAgeError, smp.TotalF, smp.TotalFError,
                          smp.K40Const, smp.K40ConstError)

def calcMDF(smp: sample.AirSample):
    smp.AirMDF, smp.AirMDF = \
        calcFuncs.get_mdf(smp.AirRatio, smp.AirRatioError, smp.Ar36Mass, smp.Ar40Mass)
...
def plotAgeSpectra(smp: sample.UnkSample):
    canvas = plotFuncs.get_default_canvas()
