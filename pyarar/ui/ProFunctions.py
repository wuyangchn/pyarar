#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @File   : welcomeWindow.py
# @Author : Yang Wu
# @Date   : 2021/5/20
# @Email  : wuy@cug.edu.cn
"""
Functions used in whole processes.
Important Constants:
...
def intercept_linest(a0: list, a1: list, *args, external=False, weight: list = None):
    ...
    n = len(args) + 2  # number of unknown x
...
def get_spectra(age: list, sage: list, Ar39k_list: list, plateau_steps: list = None, uf: int = 1, plt_sfactor: int = 1,
    ...
    '''calculate points'''
    sum_39Ark = sum(Ar39k_list); n = len(Ar39k_list)
    '''error and unit'''
    ...
    n = len(age)
    for i in range(n):
        y_uplimit = age[i] + sage[i]; y_lowlimit = age[i] - sage[i]
        k1.append(y_uplimit); k2.append(y_lowlimit); k4.append(float(Ar39k_list[i] / sum_39Ark))
        k3.append(sum(k4))
    for i in range(n + 1):
        if i == 0:
            x.append(0); y1.append(k1[i]); y2.append(k2[i])
        elif i < n:
            x.append(k3[i-1]); x.append(k3[i-1])
            if i % 2 == 1:
                y1.append(k1[i - 1]); y1.append(k2[i])
                y2.append(k2[i - 1]); y2.append(k1[i])
            elif i % 2 == 0:
                y2.append(k1[i - 1]); y2.append(k2[i])
                y1.append(k2[i - 1]); y1.append(k1[i])
        elif i == n:
            x.append(1)
            if i % 2 == 1:
                y1.append(k1[i - 1])
...
if __name__ == '__main__':
    ...
    _a = [24.48173757, 24.42967696, 24.3664485, 24.31345558, 24.26657191, 24.20899486, 24.15513883, 24.10524435, 24.05106119, 24.01051296]
    _b = [12.231848, 24.586848, 36.911848, 49.220848, 61.532848, 73.840848, 86.154848, 98.454848, 110.790848, 123.128848]
